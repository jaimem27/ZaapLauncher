...
public sealed class DownloadService
{
        ...
        double spanPercent)
    {
        ...
        for (var i = 0; i < files.Count; i++)
        {
            ...
            if (IsLocalSource(baseUrl))
            {
                ...
                await CopyLocalFileAsync(localSourcePath, fullPath, read =>
                {
                    ...
                    progress.Report(new UpdateProgress(
                        ...
                        percent,
                        "Cargando kamas al transportista…",
                        $"Copiando datos locales… ({i + 1}/{files.Count})"));
                }, ct);
    ...
    private static async Task CopyLocalFileAsync(string sourcePath, string destinationPath, Action<int> onRead, CancellationToken ct)
    {
